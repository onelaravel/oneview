# OneView Compiler - Integration with Python Compiler

## Overview

The OneView Compiler has been successfully integrated with the proven Python compiler from the onejs library. This approach allows us to leverage the sophisticated and battle-tested Python compiler (13,000+ lines) while maintaining a clean Node.js wrapper interface.

## Architecture

```
oneview/compiler/
├── python/                    # Python compiler (copied from onejs)
...
└── one.config.json          # Configuration file
```

## How It Works

### 1. .one File Processing

.one files have this structure:
```
...
1. **Parse .one File**: Extract declarations, template, script, and style sections
...
✅ **Python Compiler**: Uses battle-tested 13K+ line Python compiler from onejs
...
└── test/
    └── Demo.one               # Example .one file
```

## Example Workflow

1. Create `.one` file with template, script, and style sections
...
- The onejs Python compiler is production-tested and used by many projects
