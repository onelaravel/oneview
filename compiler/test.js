...
class CompilerTests {
    ...
    async run() {
        console.log('\n🧪 Running OneView Compiler Tests\n');
    ...
    testConfigManager() {
        ...
        this.test('Can load example config', () => {
            try {
                const configPath = path.join(__dirname, 'one.config.example.json');
    ...
    testFileDiscovery() {
        ...
        this.test('Compiler has findOneFiles method', () => {
            const compiler = new Compiler();
            if (typeof compiler.findOneFiles !== 'function') {
                throw new Error('findOneFiles method not found');
            }
        });

        this.test('Can find .one files', () => {
            const compiler = new Compiler();
            ...
            try {
                // Create test files
                fs.writeFileSync(path.join(tempDir, 'test.one'), '@useState($count, 0)\n<blade></blade>\n<script></script>');
                fs.writeFileSync(path.join(tempDir, 'other.txt'), 'not a one file');

                // Find files
                const files = compiler.findOneFiles(tempDir);
                if (files.length !== 1) {
                    throw new Error(`Expected 1 .one file, found ${files.length}`);
                }

                if (!files[0].endsWith('.one')) {
                    throw new Error('Found file is not a .one file');
                }
            } finally {
                // Cleanup
                try {
                    fs.unlinkSync(path.join(tempDir, 'test.one'));
        ...
        this.test('Returns empty array for non-existent directory', () => {
            const compiler = new Compiler();
            const files = compiler.findOneFiles('/non/existent/path/12345');
