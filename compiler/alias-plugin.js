 ...
 * 
 * Reads aliases from:
 * 1. one.config.json (aliases section)
...
/**
 * Load aliases from one.config.json
 * @param {Object} config - Loaded config object
 * @param {string} projectRoot 
 * @returns {Object} Alias map
 */
function loadOneConfigAliases(config, projectRoot) {
    const aliases = {};
    ...
    if (paths.oneView) {
        aliases['@one'] = path.resolve(projectRoot, paths.oneView);
    }
 ...
 * - Source code lives in resources/one/
 ...
 * @param {Object} config - one.config.json content
 * @param {string} projectRoot 
...
function rewriteAliasesForCompiled(aliases, config, projectRoot, contextName) {
    ...
    // Map oneView source -> compiled/context/core (if core is copied)
    if (paths.oneView) {
        const oneViewSource = path.resolve(projectRoot, paths.oneView);
        const oneViewCompiled = path.join(contextCompiledDir, 'core');
        if (fs.existsSync(oneViewCompiled)) {
            sourceToCompiled[oneViewSource] = oneViewCompiled;
        }
    }
...
/**
 * Merge aliases with priority: one.config > tsconfig > defaults
 * @param {Object} config - one.config.json content
 * @param {string} projectRoot 
...
function resolveAliases(config, projectRoot, contextName = null) {
    ...
    const oneConfigAliases = loadOneConfigAliases(config, projectRoot);
    
    // Merge: one.config takes priority
    let merged = {
        ...tsconfigAliases,
        ...oneConfigAliases
    };
