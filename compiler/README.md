...
OneView Compiler là một công cụ biên dịch chuyên biệt chuyển đổi các file template `.one` thành hai định dạng output:
...
├── config-manager.js   # Manages one.config.json
...
```
views:  paths.oneView + context.views[namespace]
        → resources/one + web/views
        → resources/one/web/views
...
### JavaScript Output (resources/one/js/temp/web/views/WebAdminUsersList.js)
```javascript
...
### Registry Output (resources/one/js/temp/web/registry.js)
```javascript
...
**Rules:**
- ✅ Filename PHẢI giống nhau (.one → .blade.php)
...
1. **KHỞI TẠO**: Đọc & parse one.config.json
2. **DUYỆT & PHÂN TÍCH**: Parse tất cả .one files
...
### Context not found
Check that context name in one.config.json matches CLI argument:
```bash
onejs-build web  # 'web' must be in config.contexts
```

### Files not being compiled
- Check file has .one extension
- Verify file is in paths specified in one.config.json
