...
class DevServer {
    ...
    async buildDev(contextName) {
        ...
        // Resolve path aliases from tsconfig.json and one.config.json
    ...
    getReloadScript() {
        return `
// OneView Dev Server - Live Reload
(function() {
    ...
    function connect() {
        ...
        es.onopen = function() {
            retryCount = 0;
            console.log('[OneView] Dev server connected');
        };
    }
...
`;
    ...
    setupWatcher(contextName) {
        ...
        const paths = this.config.paths || {};
        const oneViewPath = path.join(this.projectRoot, paths.oneView || 'resources/one');

        // Get directories to watch
        const watchDirs = [oneViewPath];
        ...
        console.log(`   📁 ${path.relative(this.projectRoot, oneViewPath)}`);
        ...
        const handleChange = async (changedPath, eventType) => {
            ...
            if (!['.one', '.ts', '.js', '.vue', '.css', '.scss'].includes(ext)) {
                return;
            }
