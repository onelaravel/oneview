...
/**
 * OneView Builder - Compile + Bundle
 * Compiles .one files and bundles app.ts into distributable JS
 */
...
class Builder {
    ...
    async setupWatcher(config, projectRoot, context, options) {
        ...
        const paths = config.paths || {};
        const oneViewPath = path.join(projectRoot, paths.oneView || 'resources/one');
        ...
        for (const ctx of contexts) {
            ...
            if (ctxConfig?.app) {
                for (const appPath of ctxConfig.app) {
                    const fullPath = path.join(oneViewPath, appPath);
                    if (fs.existsSync(fullPath)) {
                        watchDirs.push(fullPath);
                    }
                }
