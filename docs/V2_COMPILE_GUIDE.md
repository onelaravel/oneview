...
## Cấu Trúc File Input (.one)

File `.one` thường có 4 thành phần chính:
...
**Ghi chú**: File này cần được cập nhật thêm từ thư viện cũ (`/onejs/`) để bao quát đầy đủ tất cả các tính năng.
