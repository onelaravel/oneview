# OneView V2 - Blade Output Guide

## Tổng Quan

Khi compile file `.one`, phần template sẽ được sinh ra thành file Blade (`*.blade.php`) để Laravel server-side rendering.
...
- Directives chuyển từ `.one` syntax sang Blade syntax
...
Khi file `.one` khai báo `@useState`:
...
Events được khai báo trong `.one` nhưng xử lý phía client (JavaScript):
...
**File naming:**
- Input: `resources/one/app/web/views/pages/demo.one`
